      ...
      >
          ...
          {messages.map((msg) => (
            ...
            >
              ...
              <div className={`max-w-[85%] rounded-2xl p-4 text-xs leading-relaxed ${
                msg.role === 'user'
                  ? 'bg-purple-600 text-slate-900 rounded-tr-xs'
                ...
                {msg.suggestedFilter && (
                  <div className="p-3 bg-purple-950/50 border border-purple-500/40 rounded-xl space-y-2">
                    ...
                    {msg.suggestedFilter.explanation && (
                      <p className="text-slate-700 text-[11px]">{msg.suggestedFilter.explanation}</p>
                    )}
                    <button
                      ...
                      className={`w-full py-1.5 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors ${
                        appliedFilterIds.has(msg.id)
                          ...
                          : 'bg-purple-600 hover:bg-purple-500 text-slate-900 shadow-md'
                      }`}
                    >
                      {appliedFilterIds.has(msg.id) ? (
                        <>
        ...
        <div className="p-4 border-t border-slate-200 bg-white/90 flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-2.5 bg-slate-100 border border-slate-300 rounded-xl text-xs text-slate-900 placeholder-slate-500 focus:outline-none focus:border-purple-500"
          />
          <button
