...
export const AiGisModal: React.FC<AiGisModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs">
      <div 
        id="ai-gis-modal"
        className="w-full max-w-2xl bg-slate-900 border border-slate-700/80 rounded-2xl shadow-2xl overflow-hidden flex flex-col h-[600px] max-h-[92vh]"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900/90">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-lg font-semibold text-white">Assistente de Inteligência Espacial (AI GIS)</h2>
              <p className="text-xs text-slate-400">
                Camada ativa: <strong className="text-purple-300">{layer?.name || 'Nenhuma'}</strong> ({layer?.featureCount || 0} feições)
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            >
              ...
              <div className={`max-w-[85%] rounded-2xl p-4 text-xs leading-relaxed ${
                msg.role === 'user'
                  ...
                  : 'bg-slate-800/80 border border-slate-700/80 text-slate-200 rounded-tl-xs space-y-3'
              }`}>
                <div className="whitespace-pre-wrap">{msg.text}</div>

                {/* Suggested Filter action card */}
                {msg.suggestedFilter && (
                  <div className="p-3 bg-purple-950/50 border border-purple-500/40 rounded-xl space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="font-semibold text-purple-300 flex items-center gap-1.5">
                        <Sparkles className="w-3.5 h-3.5" />
                        Filtro Sugerido pela IA
                      </span>
                      <span className="text-[10px] px-2 py-0.5 bg-purple-500/20 text-purple-200 rounded-full font-mono">
                    ...
                    {msg.suggestedFilter.explanation && (
                      <p className="text-slate-300 text-[11px]">{msg.suggestedFilter.explanation}</p>
                    )}
                    <button
          ...
          {isLoading && (
            <div className="flex gap-3 justify-start">
              <div className="w-8 h-8 rounded-full bg-purple-600/20 text-purple-300 flex items-center justify-center shrink-0 animate-pulse">
                <Bot className="w-4 h-4" />
              </div>
              <div className="bg-slate-800/80 border border-slate-700/80 rounded-2xl rounded-tl-xs p-3.5 text-xs text-slate-400 flex items-center gap-2">
        ...
        <div className="px-6 py-2 bg-slate-950/60 border-t border-slate-800 flex items-center gap-2 overflow-x-auto text-[11px]">
          ...
          {quickPrompts.map((qp, idx) => (
            <button
              ...
              disabled={isLoading}
              className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded-full border border-slate-700 whitespace-nowrap transition-colors"
            >
              {qp}
            </button>
          ))}
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900/90 flex gap-2">
          ...
          <button
            ...
            className="px-4 py-2.5 bg-purple-600 hover:bg-purple-500 disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors shadow-lg shadow-purple-900/40"
          >
