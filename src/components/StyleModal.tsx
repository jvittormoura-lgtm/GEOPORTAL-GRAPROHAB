      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900/90">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-lg font-semibold text-white">Estilização da Camada: {layer.name}</h2>
              <p className="text-xs text-slate-400">Defina cores sólidas, gradientes temáticos ou mapas de calor</p>
            </div>
          </div>
                    ...
                    {PRESET_COLORS.map((c) => (
                      <button
                        ...
                        className={`w-6 h-6 rounded-md border transition-transform hover:scale-110 ${
                          style.fillColor === c ? 'ring-2 ring-sky-400 border-white' : 'border-slate-700'
                        }`}
                  ...
                  <div className="flex flex-wrap gap-1.5 flex-1">
                    {PRESET_COLORS.map((c) => (
                      <button
                        ...
                        className={`w-6 h-6 rounded-md border transition-transform hover:scale-110 ${
                          style.strokeColor === c ? 'ring-2 ring-sky-400 border-white' : 'border-slate-700'
                        }`}
                        style={{ backgroundColor: c }}
                      />
                    ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <div className="flex justify-between text-xs text-slate-300 mb-1">
                    ...
                    <span className="font-mono text-sky-400">{Math.round(style.fillOpacity * 100)}%</span>
                  </div>
                  <input
                    type="range"
                    min="0"
                ...
                <div>
                  <div className="flex justify-between text-xs text-slate-300 mb-1">
                    ...
                    <span className="font-mono text-sky-400">{style.strokeWidth}px</span>
                  </div>
                  <input
                    type="range"
                    min="1"
          ...
          {activeTab === 'thematic' && (
            <div className="space-y-5">
              <div className="flex items-center justify-between p-3.5 bg-slate-800/60 rounded-xl border border-slate-700">
                <div>
                  <h4 className="text-sm font-semibold text-white">Ativar Classificação Temática</h4>
                  <p className="text-xs text-slate-400">Colorir feições automaticamente com base em valores de atributos</p>
                </div>
                <input
                  type="checkbox"
                  ...
                  className="w-5 h-5 rounded text-sky-600 focus:ring-sky-500 bg-slate-900 border-slate-700"
                />
              </div>
              ...
              {thematicEnabled && (
                <>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="max-h-48 overflow-y-auto space-y-1.5 p-2 bg-slate-950/60 rounded-xl border border-slate-800">
                      {thematicClasses.map((cls, idx) => (
                        <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-slate-900 text-xs">
                          <div className="flex items-center gap-2.5">
                            ...
                            />
                            <span className="text-slate-200 font-medium">{cls.label}</span>
                          </div>
                          {cls.count !== undefined && (
          ...
          {activeTab === 'heatmap' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-slate-800/60 rounded-xl border border-slate-700">
                <div>
                  <h4 className="text-sm font-semibold text-white">Visualização em Mapa de Calor</h4>
                  <p className="text-xs text-slate-400">Interpola densidade e concentração espacial de pontos</p>
                </div>
                <input
                  type="checkbox"
                  checked={isHeatmap}
                  onChange={(e) => setIsHeatmap(e.target.checked)}
                  className="w-5 h-5 rounded text-amber-500 focus:ring-amber-500 bg-slate-900 border-slate-700"
                />
              </div>
        ...
        <div className="px-6 py-4 border-t border-slate-800 bg-slate-900/90 flex justify-end gap-2">
          <button onClick={onClose} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium rounded-lg">
            Cancelar
          </button>
          <button
            onClick={handleSave}
            className="px-5 py-2 bg-sky-600 hover:bg-sky-500 text-white text-xs font-semibold rounded-lg flex items-center gap-1.5 shadow-lg shadow-sky-900/40"
          >
